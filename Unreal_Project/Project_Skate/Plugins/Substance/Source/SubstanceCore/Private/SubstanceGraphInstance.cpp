...
void USubstanceGraphInstance::InitEmbeddedPresets() {
	SubstanceAir::Preset defaultPreset;
	defaultPreset.mLabel = "Default";
	defaultPreset.fill(*Instance);
	Presets.Add(FSubstancePreset(defaultPreset));
	this->AddPresets(Instance->mDesc.mPresets);
	CurrentPreset = FText::FromString(TEXT("Default"));
}

void USubstanceGraphInstance::AddPresets(const SubstanceAir::Presets& presets) {
	for (const auto& presetItr : presets) {
		FSubstancePreset preset = FSubstancePreset(presetItr);
		Presets.Add(preset);
	}
}

void USubstanceGraphInstance::PresetUpdate(const FString& presetLabel) {
	for (auto& presetItr : Presets) {
		if (presetItr.GetPresetLabel() == presetLabel) {
			presetItr.Update(this);
			CurrentPreset = FText::FromString(presetLabel);
		}
	}
}

void USubstanceGraphInstance::ApplyPreset(const FString& selectedPresetLabel) {
	for (auto& presetItr : Presets) {
		if (presetItr.GetPresetLabel() == selectedPresetLabel) {
			presetItr.Apply(this);
			CurrentPreset = FText::FromString(selectedPresetLabel);
		}
	}
	// TODO:: A problem is that when the render finishes, the thumbnails are not then 
	// regnerated. We should look into this to see if I can force refresh the content. 
	Substance::Helpers::RenderAsync(Instance);
}

FString USubstanceGraphInstance::GetCurrentPresetAsString() const {
	SubstanceAir::Preset currentValues;
	FString PresetString; 
	currentValues.fill(*Instance);
	FSubstancePreset presetHelper = FSubstancePreset(currentValues);
	return presetHelper.GetPresetString();
}

void USubstanceGraphInstance::RemovePreset(const FString& presetLabel) {
	auto pred = [presetLabel](const FSubstancePreset& inA) {
		return inA.GetPresetLabel() == presetLabel;
	};

	auto indexToRemove = Presets.IndexOfByPredicate(pred);
	while(indexToRemove != INDEX_NONE) {
		Presets.RemoveAt(indexToRemove);
		if (CurrentPreset.ToString() == presetLabel) {
			InvalidateCurrentPreset();
		}
		indexToRemove = Presets.IndexOfByPredicate(pred);
	}
}

void USubstanceGraphInstance::CreatePreset(const FString& presetName) {
	SubstanceAir::Preset currentPreset;
	currentPreset.mLabel = TCHAR_TO_ANSI(*presetName);
	currentPreset.fill(*Instance);
	Presets.Add(FSubstancePreset(currentPreset));
	CurrentPreset = FText::FromString(presetName);
}

TArray<FString> USubstanceGraphInstance::GetAllPresetLabels() {
	TArray<FString> ActivePresets;
	for (auto& presetItr : Presets) {
		ActivePresets.AddUnique(presetItr.GetPresetLabel());
	}
	return ActivePresets;
}

void USubstanceGraphInstance::InvalidateCurrentPreset() {
	CurrentPreset = FText();
}
...
void USubstanceGraphInstance::PostLoad()
{
	...
	//Update all of the presets as they only serialize the string
	for (auto& presetItr : Presets)
	{
		presetItr.GeneratePresetFromString();
	}
...
void USubstanceGraphInstance::PreSave(FObjectPreSaveContext SaveContext)
{
