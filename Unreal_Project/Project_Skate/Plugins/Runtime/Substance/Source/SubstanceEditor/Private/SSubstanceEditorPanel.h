...
private:
	...
	TMap<SubstanceAir::InputInstanceBase*, TArray<TSharedPtr<SWidget>>> InputWidgets;
	...
	TSharedPtr<SExpandableArea> PresetArea;
	TSharedPtr<SExpandableArea> OutputsArea;
	TSharedPtr<SExpandableArea> InputsArea;
	TSharedPtr<SExpandableArea> ImageInputsArea;

	TArray<TSharedPtr<STextBlock>> PresetItems;
	TSharedPtr<STileView<TSharedPtr<STextBlock>>> PresetSelectionList;
	TSharedPtr<SEditableTextBox> PresetUpdateNameInput;
	TSharedPtr<STextBlock>       PresetDeleteText;
	TSharedPtr<SEditableTextBox> PresetCreateNameInput;
	TSharedPtr<SVerticalBox> PresetConfirmUpdateBox;
	TSharedPtr<SVerticalBox> PresetConfirmDeleteBox;
	TSharedPtr<SVerticalBox> PresetConfirmCreateBox;
	TSharedPtr<SButton> PresetApplyButton;
	TSharedPtr<SButton> PresetUpdateButton;
	TSharedPtr<SButton> PresetDeleteButton;
	TSharedPtr<SButton> PresetExportButton;
	TSharedPtr<SButton> PresetCreateButton;
	TSharedPtr<SButton> PresetImportButton;
	...
	TArray<TSharedPtr<FString>> SizeComboLabels;

	TMap<FString, TArray<TSharedPtr<FString>>> ComboOptionsLabels;
	...
	TSharedRef<ITableRow> GeneratePresetWidget(TSharedPtr<STextBlock> presetName, const TSharedRef<STableViewBase>& OwnerTable);
	void OnPresetSelectionChanged(TSharedPtr<STextBlock> selectedBlock, ESelectInfo::Type SelectionType) const;
	void SetPresetButtonsEnabled(bool value);
	void InvalidateCurrentPreset();
	void OnPresetApply();
	void OnPresetUpdateRequest();
	void OnPresetUpdateCancel();
	void OnPresetUpdateConfirm();
	void OnPresetDeleteRequest();
	void OnPresetDeleteCancel();
	void OnPresetDeleteConfirm();
	void OnPresetExport();
	void OnPresetCreateRequest();
	void OnPresetCreateCancel();
	void OnPresetCreateConfirm();
	void OnPresetImport();
	...
	void OnAssetDropped(const FDragDropEvent& DragEvent, TArrayView<FAssetData> InAssets, SubstanceAir::InputInstanceBase* Input, TSharedPtr<FAssetThumbnail> Thumbnail);
